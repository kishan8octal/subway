    ...
    <RadioGroup :disabled="disabled">
        <div :class="!isSimple ? 'grid grid-cols-1 gap-y-6 sm:gap-x-2' : 'flex flex-wrap gap-1'">
            <RadioGroupOption as="template" v-for="option in options" :key="option.id" :value="option" v-slot="{ active, checked }">
                ...
                'relative flex items-center shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]  rounded-lg border bg-white px-4 py-3 focus:outline-none hover:border-green-600 hover:ring-2 hover:ring-green-600 hover:ring-opacity-20 text-xl viga-regular text-green-gray-900']">
