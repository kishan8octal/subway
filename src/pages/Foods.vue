  ...
  <section>
    ...
    <div class="p-5 z-50">
      <div class="sm:max-w-7xl mx-auto p-5 sm:p-20">
        <div
          class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-y-28 gap-x-10"
        >
          <div
            ...
            :hovered="{ scale: 1.1 }"
            class="border bg-white border-green-600 cursor-pointer ring-2 ring-green-600 ring-opacity-20 relative rounded-md shadow-lg"
          >
            <div class="rounded-full mx-auto mt-[-80px] h-[200px] w-[200px]">
              <img
                :src="card.image"
                alt="img"
                class="rounded-full mx-auto h-[180px] w-[180px] object-cover"
              />
            </div>
            <div class="p-4 text-center rounded-lg">
              <p
                class="text-[24px] font-semibold font-[cursive] uppercase mt-5"
              >
                {{ card.name }}
              </p>
              <p class="text-[18px] font-[cursive] mt-5">{{ card.dec }}</p>
                  <span class="text-[24px] font-semibold font-[cursive] text-sky-500 mt-5 drop-shadow-2xl hover:text-blue-600">{{card.price}}</span>
            </div>
          </div>
        </div>
      </div>
